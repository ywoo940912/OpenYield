      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-0.5">
            Define die/cell dimensions and yield targets for semiconductor wafers and glass panels (GCS/TGV, PLP, and display).
          </p>
        </div>
        <button
            ...
            <Section label="Identity">
              <div className="grid grid-cols-2 gap-4">
                <Field label="Spec ID *" name="spec_id" value={form.spec_id} onChange={handleChange}
                  placeholder={isPanel ? "e.g. GCS-TGV-510" : "e.g. N3-7x7"} required />
                <Field label="Product Name *" name="product_name" value={form.product_name} onChange={handleChange}
                  placeholder={isPanel ? "e.g. Glass Core Substrate TGV" : "e.g. Flagship SoC N3"} required />
              </div>
            ...
            {!isPanel ? (
              ...
              <Section label="Glass Panel Properties">
                <p className="text-xs text-slate-500">
                  Enter your panel's physical dimensions. OpenYield applies GCS/TGV defect types for glass panel inspection.
                </p>
                <div className="grid grid-cols-2 gap-4">
                  <Field label="Panel Width (mm) *" name="panel_width_mm" value={form.panel_width_mm}
                    onChange={handleChange} placeholder="e.g. 510" type="number" required />
                  <Field label="Panel Height (mm) *" name="panel_height_mm" value={form.panel_height_mm}
                    onChange={handleChange} placeholder="e.g. 515" type="number" required />
                </div>
                <div className="grid grid-cols-3 gap-4">
                  <Field label="Panel Technology" name="display_technology" value={form.display_technology}
                    onChange={handleChange as (e: React.ChangeEvent<HTMLInputElement>) => void}
                    placeholder="e.g. GCS-TGV, PLP" />
                  <Field label="Unit Width (mm) *" name="die_width_mm" value={form.die_width_mm}
                    onChange={handleChange} placeholder="e.g. 25" type="number" step="0.01" required />
                  <Field label="Unit Height (mm) *" name="die_height_mm" value={form.die_height_mm}
                    onChange={handleChange} placeholder="e.g. 25" type="number" step="0.01" required />
                </div>
              </Section>
            )}
