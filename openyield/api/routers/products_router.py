...
CREATE TABLE IF NOT EXISTS product_specs (
    ...
    wafer_diameter_mm       REAL NOT NULL DEFAULT 300.0,
    ...
    try:
        with conn:
            conn.execute(
                ...
                " wafer_diameter_mm, critical_area_fraction, target_yield, alpha, "
                " d0_target, process_node_nm, notes, created_at, updated_at) "
                "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                (
                    body.spec_id, body.product_name, body.substrate_type,
                    body.die_width_mm, body.die_height_mm, body.wafer_diameter_mm,
                    body.critical_area_fraction, body.target_yield, body.alpha,
                    body.d0_target, body.process_node_nm, body.notes, now, now,
                ),
            )
